...
def fetch_weather(latitude, longitude):
    response = requests.get(
        ...
        params={
            ...
            "forecast_days": 8,
            # Open-Meteo returns hourly timestamps in the
            # location's local timezone when timezone=auto is used.
            "timezone": "auto",
        },
        timeout=20,
    )
...
def get_api_today(data):
    """Get today's date in the weather location's timezone."""
    ...
    # Safe fallback: Open-Meteo starts forecast hourly data at
    # 00:00 local time when timezone=auto is used.
    return datetime.fromisoformat(data["hourly"]["time"][0]).date()


def rows_for_date(rows, selected_date):
    target = selected_date.isoformat()
    return [row for row in rows if row["Date"] == target]
...
- If the requested date/time is not in the supplied data, say that clearly.
...
"""
    ...
    # Stable Gemini Flash model documented by Google.
    url = (
...
st.caption(
    "City search • Today 24H • AI Assistant • 8-Day hourly forecast "
    "• Specific date • Between dates • Live"
)
...
if not available_dates:
    st.error("No hourly forecast dates were returned.")
    st.stop()

api_today = get_api_today(weather)
...
with tabs[0]:
    ...
    today_rows = rows_for_date(rows, api_today)

    if today_rows:
        st.success(
            f"Showing {len(today_rows)} hourly records for "
            f"{api_today.strftime('%A, %d %B %Y')} "
...
with tabs[1]:
    ...
    question = st.chat_input("Ask: Aaj 7 baje baarish hogi?")
...
with tabs[2]:
    ...
    for date_text in available_dates[:8]:
        selected_rows = [
            row for row in rows if row["Date"] == date_text
        ]
        selected_date = datetime.fromisoformat(date_text).date()

        with st.expander(selected_date.strftime("%A, %d %B %Y")):
            st.dataframe(
                selected_rows,
...
with tabs[3]:
    ...
    min_date = datetime.fromisoformat(available_dates[0]).date()
    max_date = datetime.fromisoformat(available_dates[-1]).date()
    ...
    selected_rows = rows_for_date(rows, selected_date)
    ...
    if selected_rows:
        ...
        st.warning("No hourly weather data was returned for this date.")

with tabs[4]:
    st.subheader("↔️ Between Dates — 24-Hour Data")
    ...
    if start_date > end_date:
        ...
        if range_rows:
            ...
            st.warning("No hourly weather data was returned for this range.")
...
with tabs[5]:
    ...
    st.write("Condition:", weather_label(current.get("weather_code")))
    st.caption(f"Location timezone: {weather.get('timezone', 'local time')}")

    if st.button("🔄 Refresh Weather"):
        st.rerun()

st.divider()
st.caption("Weather data: Open-Meteo • AI: Gemini REST API")
